...
function start(forceNewLogin) {
    ...
    async.parallel({
        getCookiesFile: function (done) {
            readFile(__dirname + '/cookies.json', function (err, cookieFileData) {
                done(err, cookieFileData);
            });
        },
        getVerificationTokenFile: function (done) {
            readFile(__dirname + '/verificationToken.txt', function (err, verificationFileData) {
        ...
        try {
            ...
            var cookieJar = request.jar(new FileCookieStore(__dirname + '/cookies.json'));
            printActualInformation(cookieJar);
        } catch (err) {
...
function createCookiesJar(callback) {
    ...
    fs.writeFile(__dirname + '/cookies.json', '', function (err) {
        if (err) {
            console.error(err);
            return;
        }
...
function rememberVerificationToken(verificationToken, callback) {
    fs.writeFile(__dirname + '/verificationToken.txt', verificationToken, function (err) {
        if (err) {
            console.error(err);
            return;
        }
